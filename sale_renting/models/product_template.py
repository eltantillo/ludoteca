...
class ProductTemplate(models.Model):
    ...
    pieces = fields.One2many('product.piece', 'product_template_id', string="Pieces")
    rent_product = fields.Many2one('product.template')

    # Delays pricing
    extra_hourly = fields.Float("Extra Hour", compute="_compute_delay")
    extra_daily = fields.Float("Extra Day", compute="_compute_delay")
    extra_hourly_percent = fields.Float("Extra Hour", company_dependent=True)
    extra_daily_percent = fields.Float("Extra Day", company_dependent=True)

    # Board Game Metadata
    players_min = fields.Integer("Min. Players")
    players_max = fields.Integer("Max. Players")
    time_min = fields.Integer("Min. Playtime")
    time_max = fields.Integer("Max. Playtime")
    age = fields.Integer("Min. Age")
    level = fields.Selection([
        ('entry', 'Entry'),
        ('intermediate', 'Intermediate'),
        ('advanced', 'Advanced'),
    ], string="Level")
    is_expansion = fields.Many2one(
        'product.template',
        string="Base Game",
        help="If this is an expansion, choose the base game here to raise a warning when renting alone. Otherwise, leave blank.")

    @api.depends('extra_hourly_percent', 'extra_daily_percent')
    def _compute_delay(self):
        for product in self:
            product.extra_hourly = (product.extra_hourly_percent * product.list_price) / 100
            product.extra_daily = (product.extra_daily_percent * product.list_price) / 100
